...
export function BulkProductCreator({ storeId, onClose, onComplete }: { storeId: string; onClose: () => void; onComplete: () => void }) {
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <div className="modal wide-modal bulk-product-modal" role="dialog" aria-modal="true" aria-labelledby="bulk-title">
        <div className="modal-head">
          <div><h2 id="bulk-title">Массовое добавление товаров</h2><p>ИИ сгруппирует фотографии. Перед созданием проверьте предложения.</p></div>
        ...
        {!drafts.length ? (
          <div className="form">
            ...
            {previews.length > 0 && <div className="bulk-image-grid">{previews.map((url, index) => <img src={url} alt={`Изображение ${index + 1}`} key={url} />)}</div>}
            ...
            <button className="primary" type="button" disabled={images.length < 2 || loading} onClick={groupImages}>{loading && <span className="spinner" />}{loading ? "ИИ группирует изображения..." : "Сгруппировать"}</button>
          </div>
        ) : (
          <div className="bulk-drafts">
            ...
            <div className="modal-actions"><button type="button" onClick={() => setDrafts([])} disabled={saving}>Назад</button><button className="primary" type="button" onClick={createProducts} disabled={saving || drafts.some((draft) => !draft.title.trim())}>{saving ? "Создаю товары..." : `Создать товары (${drafts.length})`}</button></div>
          </div>
        )}
      </div>
    </div>
